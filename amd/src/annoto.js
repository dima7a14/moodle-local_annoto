...
define([
  ...
  return {
    ...
    checkWidgetVisibility: function() {
      var config = this.config;
      var formatSelectors = [
        'body.format-tabtopics .yui3-tabview-panel',
        'body.format-grid #gridshadebox',
      ];
      var observerNodeTarget = document.querySelector(formatSelectors.join(', '));

      if (observerNodeTarget) {
        const reloadAnnoto = () => {
        ...
        observer.observe(observerNodeTarget, observerConfig);
      }
    }
  };
});
